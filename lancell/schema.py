...
class CellIndex(LanceModel):
    ...
    def validate_perturbation_lists(self) -> Self:
        ...
        non_none = [l for l in lists if l is not None]
        if non_none and len(set(len(l) for l in non_none)) > 1:
    ...
    def generate_perturbation_search_tokens(record: Self) -> str:
        ...
        tokens: list[str] = []
        for uid, ptype in zip(record.perturbation_uids or [], record.perturbation_types or []):
...
class DatasetPerturbationIndex(LanceModel):
    ...
    def generate_uid(self) -> Self:
        ...
        key = f"{self.dataset_uid}_{self.perturbation_uid}".encode("utf-8")
