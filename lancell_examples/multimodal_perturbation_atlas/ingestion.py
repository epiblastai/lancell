...
Includes:
- Multimodal batch ingestion (gene_expression + protein_abundance in one pass)
- Chromatin accessibility fragment ingestion

Chromatin accessibility parses gzipped 4-column BED files (chrom, start, end,
cell_barcode), sorts fragments into cell order and genome order, and writes the
parallel zarr arrays expected by ``CHROMATIN_ACCESSIBILITY_SPEC``:
...
def parse_bed_fragments(path: Path) -> pl.DataFrame:
    ...
    path
        ...
        ``chrom``, ``start``, ``end``, ``guide_sequence``.
    ...
    df = pl.read_csv(
        ...
        separator="\t",
        has_header=False,
        new_columns=["chrom", "start", "end", "guide_sequence"],
...
def sort_fragments_by_cell(
    ...
    tuple of (chromosomes, starts, lengths, offsets, cell_ids)
        ...
        cell_ids : list[str]
            Ordered list of guide_sequence strings (one per cell).
    """
    ...
    # Build deterministic cell ordering from unique guide sequences
    cell_ids: list[str] = df["guide_sequence"].unique().sort().to_list()
    n_cells = len(cell_ids)
    guide_to_idx = {g: np.uint32(i) for i, g in enumerate(cell_ids)}

    # Map guide_sequence → cell_idx and sort
    df = (
        df.with_columns(
            pl.col("guide_sequence")
            .replace_strict(guide_to_idx, return_dtype=pl.UInt32)
            .alias("cell_idx"),
        )
        .drop("guide_sequence")
