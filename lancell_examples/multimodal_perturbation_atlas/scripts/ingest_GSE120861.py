"""Ingest prepared GSE120861 (Gasperini et al. 2019) into a RaggedAtlas.

This dataset contains three CRISPR-based genetic screens profiling gene
expression via single-cell RNA-seq in K562 cells:
  - at_scale_screen   (207,324 cells × 13,135 genes)
  - pilot_highmoi_screen (47,650 cells × 32,738 genes)
  - pilot_lowmoi_screen  (41,284 cells × 32,738 genes)

Prerequisites:
  - Prepared data in /home/ubuntu/geo_agent_resolution/GSE120861/ (from geo-data-preparer)

Usage:
    python -m lancell_examples.multimodal_perturbation_atlas.scripts.ingest_GSE120861 \
        --atlas-path /tmp/atlas/GSE120861 [--limit 1000]
"""
...
ACCESSION = "GSE120861"
ACCESSION_DIR = Path("/home/ubuntu/geo_agent_resolution/GSE120861")
FEATURE_SPACE = "gene_expression"

EXPERIMENTS = ["at_scale_screen", "pilot_highmoi_screen", "pilot_lowmoi_screen"]
...
def assemble_obs(experiment: str) -> Path:
    ...
    This dataset has a mismatch: the ontology fragment is indexed by sample
    (non-unique), while perturbation and preparer fragments are indexed by
    cell_barcode (unique). We handle this by joining the ontology fragment
    via the sample column from raw_obs, then concatenating with the
    cell_barcode-indexed fragments.
    """
    ...
    # Load raw_obs to get cell_barcode → sample mapping
    raw_obs = pd.read_csv(
        exp_dir / f"{FEATURE_SPACE}_raw_obs.csv",
        usecols=["sample", "cell_barcode"],
    )
    cell_barcodes = raw_obs["cell_barcode"].values

    # Load ontology fragment (indexed by sample, non-unique)
    ont_path = exp_dir / f"{FEATURE_SPACE}_fragment_ontology_obs.csv"
    ont_df = pd.read_csv(ont_path, index_col=0)
    # Deduplicate to one row per sample, then broadcast to cell level
    ont_unique = ont_df.drop_duplicates()
    # Drop the 'sample' column from ont_unique if present (it's the index)
    ont_cols = [c for c in ont_unique.columns if c != "sample"]
    ont_unique = ont_unique[ont_cols]
    # Join: raw_obs.sample → ont_unique (indexed by sample)
    ont_broadcast = raw_obs[["sample"]].join(ont_unique, on="sample").drop(columns=["sample"])
...
def validate_obs(experiment: str) -> Path:
    ...
    subprocess.run(
        [
            ...
            "--column", "assay=Perturb-Seq",
            "--column", "organism=Homo sapiens",
            "--column", "cell_line=K-562",
        ],
        check=True,
    )
    return validated_obs
...
def load_mtx_as_anndata(
    ...
    exp_dir = ACCESSION_DIR / experiment
    mtx_path = exp_dir / f"GSE120861_{experiment}.exprs.mtx.gz"

    # Some GEO files have .gz extension but are actually plain text
    # Try reading directly first; if gzip fails, rename and retry
    print(f"  Loading expression matrix: {mtx_path.name} ...")
    plain_path = mtx_path.with_suffix("") if mtx_path.suffix == ".gz" else mtx_path
    if not (plain_path.exists() or plain_path.is_symlink()) and mtx_path.exists():
        # Symlink without .gz so scipy doesn't try gzip decompression
        plain_path.symlink_to(mtx_path)
    mat = sio.mmread(str(plain_path))
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Ingest GSE120861 into a RaggedAtlas"
    )
    parser.add_argument(
        "--atlas-path",
        type=str,
