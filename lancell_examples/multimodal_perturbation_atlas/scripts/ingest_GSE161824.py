"""Ingest prepared GSE161824 (Ursu et al. 2022) into a RaggedAtlas.

This dataset contains Perturb-seq profiling of ~200 TP53 and KRAS coding
variants in A549 lung cancer cells (~176k cells total):
  - A549_KRAS (~90k cells × 1145 genes)
  - A549_TP53 (~86k cells × 1145 genes)

Prerequisites:
  - Prepared data in /home/ubuntu/geo_agent_resolution/GSE161824/ (from geo-data-preparer)

Usage:
    python -m lancell_examples.multimodal_perturbation_atlas.scripts.ingest_GSE161824 \
        --atlas-path /tmp/atlas/GSE161824 [--limit 1000]
"""
...
ACCESSION = "GSE161824"
ACCESSION_DIR = Path("/home/ubuntu/geo_agent_resolution/GSE161824")
FEATURE_SPACE = "gene_expression"

EXPERIMENTS = ["A549_KRAS", "A549_TP53"]
...
def load_mtx_as_anndata(
    ...
    The cells/genes CSVs are headerless (one entry per line). The MTX matrix
    may be either cells×genes or genes×cells; we detect and transpose if needed.
    """
    exp_dir = ACCESSION_DIR / experiment
    mtx_path = exp_dir / f"GSE161824_{experiment}.processed.matrix.mtx.gz"
    cells_path = exp_dir / f"GSE161824_{experiment}.processed.cells.csv.gz"
    genes_path = exp_dir / f"GSE161824_{experiment}.processed.genes.csv.gz"
    ...
    # Load headerless barcodes and gene names
    mtx_barcodes = pd.read_csv(cells_path, header=None)[0].values
    mtx_genes = pd.read_csv(genes_path, header=None)[0].values
    n_barcodes = len(mtx_barcodes)
    n_genes = len(mtx_genes)

    # Detect orientation: if rows == n_genes and cols == n_barcodes, transpose
    if mat.shape == (n_genes, n_barcodes):
        print(f"  Transposing matrix from genes×cells ({mat.shape}) to cells×genes")
        mat = mat.T.tocsr()
    elif mat.shape != (n_barcodes, n_genes):
        raise ValueError(
            f"Matrix shape {mat.shape} doesn't match cells ({n_barcodes}) × genes ({n_genes})"
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Ingest GSE161824 into a RaggedAtlas"
    )
    parser.add_argument(
        "--atlas-path",
        type=str,
