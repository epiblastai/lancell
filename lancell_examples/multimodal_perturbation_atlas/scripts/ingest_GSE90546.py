"""Ingest prepared GSE90546 (Dixit et al. 2016) into a RaggedAtlas.

This dataset contains three Perturb-seq CRISPR screens profiling gene
expression via single-cell RNA-seq in K562 cells:
  - pilot_tf          (5,768 cells × 35,635 genes)
  - epistasis          (15,006 cells × 32,738 genes)
  - upr_perturb_seq   (65,337 cells × 32,738 genes)

Prerequisites:
  - Prepared data in /home/ubuntu/geo_agent_resolution/GSE90546/ (from geo-data-preparer)

Usage:
    python -m lancell_examples.multimodal_perturbation_atlas.scripts.ingest_GSE90546 \
        --atlas-path /tmp/atlas/GSE90546 [--limit 1000]
"""
...
ACCESSION = "GSE90546"
ACCESSION_DIR = Path("/home/ubuntu/geo_agent_resolution/GSE90546")
FEATURE_SPACE = "gene_expression"

# experiment_name -> (gsm_accession, 10x_batch)
EXPERIMENTS = {
    "pilot_tf": ("GSM2406675", "10X001"),
    "epistasis": ("GSM2406677", "10X005"),
    "upr_perturb_seq": ("GSM2406681", "10X010"),
}
...
def load_mtx_as_anndata(
    ...
    exp_dir = ACCESSION_DIR / experiment
    gsm, batch = EXPERIMENTS[experiment]

    # Find the matrix file: GSM*_10X*_matrix.mtx.txt.gz
    mtx_path = exp_dir / f"{gsm}_{batch}_matrix.mtx.txt.gz"

    print(f"  Loading expression matrix: {mtx_path.name} ...")
    mat = sio.mmread(str(mtx_path))
    # MTX files from GEO are gene × cell — transpose to cell × gene
    if sp.issparse(mat):
        mat = mat.T.tocsr()
    else:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Ingest GSE90546 into a RaggedAtlas"
    )
    parser.add_argument(
        "--atlas-path",
        type=str,
