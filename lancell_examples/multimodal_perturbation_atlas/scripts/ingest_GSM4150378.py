"""Ingest prepared GSM4150378 (Srivatsan et al. 2019) into a RaggedAtlas.

sciPlex3 dataset: single-nucleus RNA-seq of A549, MCF7, and K562 cancer
cell lines exposed to 188 small molecule inhibitors or DMSO vehicle control.

Data format: cell-sorted COO triplet matrix (gene_idx, cell_idx, count),
gzipped, 1-indexed, tab-separated. 762,795 cells × 110,983 genes.
138 genes share a global_feature_uid with another gene, so deduplication
...
Usage:
    python -m lancell_examples.multimodal_perturbation_atlas.scripts.ingest_GSM4150378 \
        --atlas-path /tmp/atlas/GSM4150378 [--limit 1000]
"""
...
def load_coo_as_anndata(
    ...
    Reads the COO file via polars streaming, builds a scipy CSR matrix,
    and deduplicates features that share a global_feature_uid (summing counts).
    """
    print(f"  Loading COO file: {coo_path.name} ...")

    # Read COO triplets via polars streaming (handles gzip natively)
    lf = pl.scan_csv(coo_path, has_header=False, separator="\t")

    if limit is not None and limit < n_cells:
        # Filter to first `limit` cells (1-indexed in file)
        lf = lf.filter(pl.col("column_2") <= limit)
        n_cells = limit
...
def ingest_experiment(
    ...
    # Apply limit to obs
    if limit is not None and limit < n_cells_total:
        print(f"  Limiting to {limit} cells (of {n_cells_total})")
        obs_df = obs_df.iloc[:limit]

    n_cells = len(obs_df)
    print(f"  {experiment}: {n_cells:,} cells × {n_features:,} features")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Ingest GSM4150378 into a RaggedAtlas"
    )
    parser.add_argument(
        "--atlas-path",
        type=str,
